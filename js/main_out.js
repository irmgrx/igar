...
function scriptStart() {
    (function(d, e) {
        ...
        function initLayout() {
            $ = e;
            // downloadFile(removetagFile, function(response){
            //   var removeJson = JSON.parse(response.responseText);
            //   var i = 0
            //   for(i = 0; i < removeJson.remove.length; i++){
            //     $(removeJson.remove[i]).remove();
            //   }
            // });
            // var mainPanelFile = gapi.client.drive.files.get({
            //   'fileId': MAINPANELFILEID
            // });
            // downloadFile(mainPanelFile, function(response){
            //   $("#mainPanel").remove();
            //   $("#stats").before($(response.responseText));
            //   $(".agario-profile-panel").attr('background', "#2D3031");
            // });
        }
        ...
        function fc() {
            if (.4 > g) ia = null;
                ...
                for (h = 0; h < x.length; h++)
                    if (e = x[h], e.H() && !(20 >= e.size * g))
                        for (a = 0; a < e.a.length; ++a) c = e.a[a].x, b = e.a[a].y, c < v - m / 2 / g || b < w - q / 2 / g || c > v + m / 2 / g || b > w + q / 2 / g || ia.Z(e.a[a])
            }
        }
        ...
        function ec() {
            var a = -1 != d.location.search.indexOf("fb"),
                a = za() && !a;
        ...
        function Ya(a) {
            ...
            for (var c = 0; c < a.length; c++)
                if (!("type" in a[c] && "rubicon" != a[c].type)) {
                    var b = a[c].data;
        ...
        function cc() {
            var a = {},
                ...
                s = null,
                h = null,
                e = -1 != d.location.search.indexOf("fb"),
                f = za() && !e,
                n = ya.w;
        ...
        function lc(a, c) {
            ...
            c += 2;
            for (var f = 0; f < h; ++f) {
                var p = M[a.getUint32(c, !0)],
                    n = M[a.getUint32(c + 4, !0)];
                c += 8;
            ...
            for (f = 0;;) {
                ...
                l.U = X;
                r && (l.J = r);
                t && l.q(t); - 1 != C.indexOf(h) && -1 == k.indexOf(l) && (k.push(l), 1 == k.length && (v = l.x, w = l.y, Mb(), document.getElementById("overlays").style.display = "none", z = [], jb = 0, kb = k[0].color, Da = !0, lb = Date.now(), Y = La = mb = 0))
            }
            p = a.getUint32(c, !0);
            c += 4;
        ...
        function Rb() {
            ...
            tc();
            c = Date.now() - c;
            c > 1E3 / 60 ? I -= .01 : c < 1E3 / 65 && (I += .01);.4 > I && (I = .4);
            1 < I && (I = 1);
            c = H - Ub;
        ...
        function Kb() {
            D = null;
            if (null != E || 0 != y.length)
                if (null != E || Pa) {
                    D = document.createElement("canvas");
                    var a = D.getContext("2d"),
                        c = 60,
                        c = null == E ? c + 24 * y.length : c + 180,
                    ...
                    if (null == E)
                        for (a.font = "20px Ubuntu", c = 0; c < y.length; ++c) b = y[c].name || ma("unnamed_cell"), Pa || (b = ma("unnamed_cell")), -1 != C.indexOf(y[c].id) ? (k[0].name &&
                            (b = k[0].name), a.fillStyle = "#FFAAAA") : a.fillStyle = "#FFFFFF", b = c + 1 + ". " + b, a.fillText(b, 100 - a.measureText(b).width / 2, 70 + 24 * c);
                    else
                        for (c = b = 0; c < E.length; ++c) {
        ...
        function da(a, c) {
            ...
            if (b) {
                var h = +e(".agario-exp-bar .progress-bar-text").first().text().split("/")[0],
                    b = +e(".agario-exp-bar .progress-bar-text").first().text().split("/")[1].split(" ")[0],
                    s = e(".agario-profile-panel .progress-bar-star").first().text();
                if (s != a.level) da({
                    xp: b,
                    xpNeeded: b,
                    level: s
        ...
        function zc() {
            ...
            for (var a = 0; a < y.length; ++a)
                if (-1 != C.indexOf(y[a].id)) return a + 1;
            return 0
        }
        ...
        if (!d.agarioNoInit) {
            var tb = d.location.protocol,
                db = "https:" == tb;
            B.master && (L.master_url = B.master);
            var ja = tb + "//" + L.master_url + "/",
                Qa = d.navigator.userAgent;
            if (-1 != Qa.indexOf("Android")) d.ga && d.ga("send", "event", "MobileRedirect", "PlayStore"), setTimeout(function() {
            ...
            else if (-1 != Qa.indexOf("iPhone") || -1 != Qa.indexOf("iPad") || -1 != Qa.indexOf("iPod")) d.ga && d.ga("send", "event", "MobileRedirect", "AppStore"), setTimeout(function() {
                d.location.href = "https://itunes.apple.com/app/agar.io/id995999703?mt=8&at=1l3vajp"
            }, 1E3);
            else {
                var Sa, f, P, m, q, ia = null,
                    r = null, //WebSocket object
                    v = 0, //your mass X Position
                    w = 0, //your mass Y position
                    C = [],
                    k = [], // mycells
                    M = {}, // players by id
                    x = [], // players that around you
                    ca = [],
                    y = [], //leaderboard player
                    ta = 0, //mouse X position related to screen
                    ua = 0, //mouse Y position related to screen
                    Aa = -1,
                    Ba = -1,
                    rc = 0,
                    H = 0,
                    ...
                    A = null,
                    ub = !1, //check box skin
                    Pa = !0, //check box show name
                    vb = !1, //check box show colors
                    ib = !1,
                    N = 0, // Player score
                    Ma = !0, // check box dark theme
                    Zb = !0, // check box show mass
                    oa = v = ~~((Ha + Ja) / 2),
                    pa = w = ~~((Ia + Ka) / 2),
                    qa = 1,
                    T = "",
                    ...
                    Fa = 0,
                    Ga = 0,
                    uc = ["#333333", "#FF3333", "#33FF33", "#3333FF"],
                    ob = !1,
                    na = !1,
                    Lb = 0,
                    F = null,
                ...
                var xb = "ontouchstart" in d && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(d.navigator.userAgent),
                    pb = new Image;
                pb.src = "/img/split.png";
                var $b = document.createElement("canvas");
                if ("undefined" == typeof console || "undefined" == typeof DataView || "undefined" == typeof WebSocket || null == $b || null == $b.getContext || null == d.localStorage) alert("You browser does not support this game, we recommend you to use Firefox to play this");
                else {
                    var Ca = null;
                    ...
                    e.get(tb + "//gc.agar.io", function(a) {
                        var c = a.split(" ");
                        a = c[0];
                        c = c[1] || ""; - 1 == ["UA"].indexOf(a) && ac.push("ussr");
                    ...
                    var $a = !1,
                        ...
                        ba = function(a) {
                            a.defineSlot = function(a, b, e) {
                                ...
                                return {
                                    type: "dfpads",
                                    data: h.defineSlot(a, b, e).addService(h.pubads())
                                }
                            };
                            return a
                        }({});
                    ...
                    var D = null,
                        ...
                        ga = {},
                        ac = "poland;usa;china;russia;canada;australia;spain;brazil;germany;ukraine;france;sweden;chaplin;north korea;south korea;japan;united kingdom;earth;greece;latvia;lithuania;estonia;finland;norway;cia;maldivas;austria;nigeria;reddit;yaranaika;confederate;9gag;indiana;4chan;italy;bulgaria;tumblr;2ch.hk;hong kong;portugal;jamaica;german empire;mexico;sanik;switzerland;croatia;chile;indonesia;bangladesh;thailand;iran;iraq;peru;moon;botswana;bosnia;netherlands;european union;taiwan;pakistan;hungary;satanist;qing dynasty;matriarchy;patriarchy;feminism;ireland;texas;facepunch;prodota;cambodia;steam;piccolo;ea;india;kc;denmark;quebec;ayy lmao;sealand;bait;tsarist russia;origin;vinesauce;stalin;belgium;luxembourg;stussy;prussia;8ch;argentina;scotland;sir;romania;belarus;wojak;doge;nasa;byzantium;imperial japan;french kingdom;somalia;turkey;mars;pokerface;8;irs;receita federal;facebook;putin;merkel;tsipras;obama;kim jong-un;dilma;hollande;berlusconi;cameron;clinton;hillary;venezuela;blatter;chavez;cuba;fidel;merkel;palin;queen;boris;bush;trump".split(";"),
                        Bc = "8;nasa;putin;merkel;tsipras;obama;kim jong-un;dilma;hollande;berlusconi;cameron;clinton;hillary;blatter;chavez;fidel;merkel;palin;queen;boris;bush;trump".split(";"),
                        ra = {};
                    qb.prototype = {
                        Q: null,
                        x: 0,
                    ...
                    ea.prototype = {
                        ...
                        S: function() {
                            ...
                            delete M[this.id];
                            a = k.indexOf(this); - 1 != a && (ib = !0, k.splice(a, 1));
                        ...
                        ea: function() {
                            ...
                            for (var a = this.a, c = a.length, b = 0; b < c; ++b) {
                                ...
                                a[b].b *= .7;
                                10 < a[b].b && (a[b].b = 10); - 10 > a[b].b && (a[b].b = -10);
                        ...
                        K: function() {
                            ...
                            this.f();
                            if (this.u && 1 <= c) {
                                var b = ca.indexOf(this); - 1 != b && ca.splice(b, 1)
                            }
                        ...
                        p: function(a) {
                            if (this.H()) {
                                ...
                                if (this.M && !c)
                                    for (var b = 0; b < this.a.length; b++) this.a[b].d = this.size;
                                this.M = c;
                                a.save();
                                this.X = H;
                                b = this.K();
                                ...
                                if (c) a.beginPath(), a.arc(this.x, this.y, this.size + 5, 0, 2 * Math.PI, !1);
                                else {
                                    this.ea();
                                    a.beginPath();
                                    var e = this.v();
                                    ...
                                    fa: function(a, d, e, f, h) {
                                        ...
                                        for (; p <= d; ++p)
                                            for (f = g; f <= a; ++f)
                                                if (e = l[f + p * n], null != e)
                                                    if (Array.isArray(e))
                                                        for (var m = 0; m < e.length; m++) h(e[m]);
                                                    else h(e)
                                    }
                                }
                            }
                    ...
                    d.fbAsyncInit = function() {
                        ...
                        d.FB.Event.subscribe("auth.statusChange", function(c) {
                            console.log("window.localStorage['wannaLogin'] = " + d.localStorage.wannaLogin); + d.localStorage.wannaLogin && ("connected" == c.status ? rb(c) : (console.log("user not connected, facebookLogin()"), a()))
                        });
                        d.facebookRelogin = a;
                        d.facebookLogin = a
                    };
